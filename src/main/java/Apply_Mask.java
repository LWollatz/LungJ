...
/***
 * Apply_Mask applies a mask to an image, leaving the foreground as it is and
 * replacing the background by black.
 * 
 * - As a preparation, make sure the mask is binary where the background is
 * minimum and the foreground maximum. - One image should represent the mask,
 * while the other one should represent the original data. - Use Apply Mask to
 * replace background values in the original data with black.
 * 
...
public class Apply_Mask implements PlugIn {
	...
	public void run(String command) {
		...
		for (int i = 0; i < Nimg; i++) {
			ImageProcessor temp = WindowManager.getImage(lstImageIds[i])
					.getProcessor();
		...
		/** create output image **/
		ImagePlus imgout = IJ.createHyperStack("Result", tWidth, tHeight,
				tChannels, tDepth, tFrames, 8);

		/** process image **/
		for (int z = 1; z <= tDepth; z++) {
			ImageProcessor imageIP = image.getStack().getProcessor(z)
					.convertToByte(true);
			for (int f = 1; f <= tFrames; f++) {
				for (int c = 1; c <= tChannels; c++) {
					int index = mask.getStackIndex(c, z, f);
					ImageProcessor maskIP = mask.getStack().getProcessor(index)
							.convertToByte(true);
					String label = mask.getStack().getShortSliceLabel(index);
					ImageProcessor outputIP = imgout.getStack().getProcessor(
							index);
