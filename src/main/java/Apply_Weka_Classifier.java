...
/***
 * Apply_Weka_Classifier applies a Weka classifier based on the filename of an
 * image and the filename of a classifier model. This implements the whole
 * application process in a single function and does not require loading the
 * original image into memory twice. This function is likely to suffer from
 * updates to the weka-segmentation plug-in but will hopefully be directly
 * implemented by the trainable segmentation in a future version.
 * 
 * - Pre-process the image of interest according to your needs. Samples must
 * have the correct resolution (bit-size) and value range for the classifier you
 * want to use. There is no need to apply a noise filter, as the Weka
 * segmentation does this for you. - Select the location of the image you want
 * to segment. It is not advised to load the image in ImageJ prior to calling
 * this function, as that requires extra memory. - Select a classifier model
 * from your files. LungJ offers some sample classifiers which currently have to
 * be downloaded separately from the java plug in. - Choose the class you want
 * to extract. Each classifier has at least 2 classes (foreground and
 * background). Future versions are planned to allow the choice of several
 * classes. - Press `OK’ and the Weka segmentation will be started, the image
 * and the classifier loaded and eventually the classifier will be applied. This
 * process can take a long time and the progress bar updates by the Trainable
 * Weka Segmentation will sometimes suggest that it has finished, even-though it
 * is still working. Apply_Weka_Classifier will close the Trainable Weka
 * Segmentation window, once the segmentation has completed.
 *
 * @author Lasse Wollatz
 * 
 * @see <a href="http://imagej.net/Trainable_Weka_Segmentation">Trainable Weka
 *      Segmentation</a>
 ***/
public class Apply_Weka_Classifier implements PlugIn, ActionListener {
	 ...
	 **/
	public static String TWS_version = "v" + PLUGIN_TWS; // as this is included
															// in the window
															// name...
	...
	public void run(String command) {
		...
		if (IJ.isMacro() && Macro.getOptions() != null
			...
			LJ_srcDirectory = LJPrefs.retrieveOption(arguments, "filepath",
					LJ_srcDirectory);
			LJ_clsDirectory = LJPrefs.retrieveOption(arguments, "classifier",
					LJ_clsDirectory);
			...
			JLabel clsdirlbl = new JLabel("Classifier Directory  ",
					JLabel.RIGHT);
			...
			// TODO: remove and reorder block to be ready for 4D visualisation
			// instead.
			...
			IJ.showStatus("Initializing...");
			// TODO: recorder needs to get escape characters correctly! i.e.
			// "\\\\"->\\ not "\\"->\
		...
		// "open=["+LJ_srcDirectory+"\\"+LJ_srcFilename+"]");
		// IJ.run("Trainable Weka Segmentation",
		// "open=["+LJ_srcDirectory+"\\"+LJ_srcFilename+"] inputfile=["+LJ_srcDirectory+"\\"+LJ_srcFilename+"] path=["+LJ_srcDirectory+"]");
		// IJ.log("open=["+LJ_srcDirectory+"] inputfile=["+LJ_srcDirectory+"] path=["+LJ_srcPath+"]");
		// IJ.run("Trainable Weka Segmentation",
		// "open=["+LJ_srcDirectory+"] inputfile=["+LJ_srcDirectory+"] path=["+LJ_srcPath+"]");
		...
		for (int i = 0; i < Nimg; i++) {
			if (lstImages[i].startsWith("Trainable Weka Segmentation")) {
				...
				if (!(TWS_version.equals(tempStr))) {
					IJ.log("Newer version of the Trainable WEKA Segmentation found (found "
							+ tempStr
							+ ", expected "
							+ TWS_version
		...
		IJ.runMacro("selectWindow('Trainable Weka Segmentation "
				+ TWS_version
				+ "'); call('trainableSegmentation.Weka_Segmentation.loadClassifier', '"
				+ LJ_clsDirectory.replace("\\", "\\\\") + "');");
		...
		// LJ_srcDirectory, LJ_srcFilename, "showResults=true",
		// "storeResults=false", "probabilityMaps=true", "");
		IJ.runMacro("call('trainableSegmentation.Weka_Segmentation.getProbability');");
		IJ.showProgress(90, 100);
		/*** post processing ***/
		IJ.showStatus("Finishing...");
		IJ.runMacro("selectWindow('Trainable Weka Segmentation " + TWS_version
				+ "'); close(); selectWindow('Probability maps');");
		/*** remove background channel ***/
		// TODO: NEED TO MAKE THIS INDEPENDANT OF IMAGE SIZE!
		ImagePlus image = WindowManager.getCurrentImage();
		int[] properties = image.getDimensions(); // width, height, nChannels,
													// nSlices, nFrames
		...
		IJ.run("Slice Remover", "first=" + AC_channel + " last=" + nSlices
				+ " increment=" + nChannels);
	...
	public void actionPerformed(ActionEvent e) {
		/**
		 * code to display file selection dialogs and store chosen result back
		 * into textbox
		 **/
		if (e.getSource() == this.clsfilebtn) {
			JFileChooser chooser = new JFileChooser(LJ_clsDirectory);
			FileFilter filter = new FileNameExtensionFilter("Classifier Model",
					"model");
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				this.clsdirtxt.setText(chooser.getSelectedFile()
						.getAbsolutePath());
			}
		}
		if (e.getSource() == this.srcfilebtn) {
			JFileChooser chooser = new JFileChooser(LJ_srcDirectory);
			FileFilter tiffilter = new FileNameExtensionFilter(
					"tif images (*.tif, *.tiff)", "tif", "tiff");
			FileFilter rawfilter = new FileNameExtensionFilter(
					"raw data (*.raw)", "raw");
